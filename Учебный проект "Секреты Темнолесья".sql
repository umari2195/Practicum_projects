...
-- Для начала проводим ИАД и анализируем доли платящих игроков
...
SELECT race,
       ...
       ORDER BY sum DESC;
-- Исследуем внутриигровые покупки:
-- 1. Статистические показатели по полю amount:
...
total_sale AS (
    ...
    SELECT 
        COUNT(z.amount) AS total_sales_amount,
        (SELECT COUNT(DISTINCT id) FROM fantasy.events) AS total_players 
    FROM zero_sale AS z
),
count_sale AS (
    ...
    SELECT 
        z.item_code,
        COUNT(z.amount) AS item_sales_amount, 
        COUNT(DISTINCT z.player_id) AS count_player 
    FROM zero_sale AS z
...
-- Изучаем зависимость активности игроков от расы персонажа:
WITH alies_1 AS(
SELECT race_id,
...
SELECT a1.race_id,
       ...
       buyer_players/total_players ::REAL AS dolya_1,
